...
class MD5Calculator {
	public:
		explicit MD5Calculator(const char *s, int len)
		{
			assert(s && len);
			calculate(s,len);
		}
		explicit MD5Calculator(const std::string& s);
		string hex();
	protected:
		calculate(const char *s, int len)
		{
			const char *str = s;
			unsigned char buf[16];
			for(str = s;str <= s-16; str+=16)
			{
				update(str);
			}
		}
		memset(buf,0,16);
		update(const char *buf);
};
